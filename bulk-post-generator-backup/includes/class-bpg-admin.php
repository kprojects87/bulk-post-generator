...
class BPG_Admin {
	...
	public function maybe_save_settings() {
		if ( ! isset( $_POST['bpg_settings_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['bpg_settings_nonce'] ) ), 'bpg_save_settings' ) ) {
			return;
		}
		...
		$settings = array(
			...
			'word_count'        => sanitize_text_field( wp_unslash( $_POST['word_count'] ?? 'medium' ) ),
	...
	private function render_history_tab() {
		$query = new WP_Query(
			array(
				'post_type'      => 'post',
				'posts_per_page' => 20,
				'meta_query'     => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Small, low-frequency admin screen query; no better index available for this custom flag.
					array(
						'key'     => '_bpg_generated',
						'value'   => 1,
						'compare' => '=',
